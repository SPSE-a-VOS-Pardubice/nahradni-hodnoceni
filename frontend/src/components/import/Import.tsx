...
const Import = (props: {
    ...
    setPhase: (newPhase: ImportPhase) => void
}) => {
  useEffect(() => {
    if (props.phase.phase === '2_UPLOADING') {
      uploadCsvFile(props.phase.data).then(response => {
        if (props.phase.phase !== '2_UPLOADING') { throw new Error('Invalid state: import phase changed while uploading data.'); }
            ...
            <Popup open={true} options={[
              {
                ...
                onClick: async () => {
                  ...
                  if (props.phase.phase !== '3_MISSING_EXAMINERS') { throw new Error('Invalid state: import phase changed while uploading data.'); }

                  examinerPrefixEl.value = '';
                  examinerNameEl.value = '';
                  examinerSuffixEl.value = '';

                  const newPhase = {...props.phase};
            ...
            <Popup open={true} options={[
              {
                ...
                onClick: async () => {
                  ...
                  if (props.phase.phase !== '4_MISSING_SUBJECTS') { throw new Error('Invalid state: import phase changed while uploading data.'); }
